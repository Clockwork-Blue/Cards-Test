...
function generator() {
 let mon = document.getElementById("name").innerHTML = "u/" + adjective[Math.floor(Math.random() * adjective.length)] + " " + object[Math.floor(Math.random() * object.length)];
 document.getElementById("name").title= mon;
}
...
function getRndImg(){
    let number = getRndInteger(1,225);
    let sourceImg= "img/img ("+ number +").jpg";
    document.getElementById("card-img").src =sourceImg;
}
